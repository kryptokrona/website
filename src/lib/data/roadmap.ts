export type RoadmapData = {
  title: string
  time: string
  description: string
}

export const roadmap: RoadmapData[] = [
  {
    "title": "Revamp of website",
    "time": "2022 Q2",
    "description": "A new fancy website for XKR was released."
  },
  {
    "title": "Hugin Cache 1.0",
    "time": "2022 Q2",
    "description": "The Hugin cache can store messages that nodes have discarded. Kind of like an answering machine for Hugin Messenger."
  },
  {
    "title": "Munin 1.0",
    "time": "2022 Q2",
    "description": "Munin is a scalability upgrade that vastly improves the capabilities of the XKR network and makes sending messages on Hugin free!"
  },
  {
    "title": "Hugin Svelte Alpha",
    "time": "2022 Q3",
    "description": "A revamped Hugin Desktop app based on the Svelte UI framework was released in an alpha stage."
  },
  {
    "title": "Hugin Mobile First Release",
    "time": "2022 Q3",
    "description": "Messaging happens on mobile, and with the release of Hugin Mobile for Android, your mobile messaging has never been more secure!"
  },
  {
    "title": "New Docs",
    "time": "2022 Q3",
    "description": "A new documentation site hosted on GitBook was released."
  },
  {
    "title": "Hugin Svelte Release",
    "time": "2022 Q4",
    "description": "A revamped Hugin Desktop app based on the Svelte UI framework was released."
  },
  {
    "title": "Filesharing Hypercore Hugin Desktop",
    "time": "2023 Q1",
    "description": "As a push to explore new and exciting technologies our engineers have done some trials with a new protocol for file sharing called Hypercore. Although very capable, it is not yet ready for mobile platforms."
  },
  {
    "title": "Hugin Full node (Node + Hugin API)",
    "time": "2023 Q1",
    "description": "Hugin full node combines the XKR node and Hugin API, as well as some other neat features into one easy to setup \"full node\". With this technology we can make the user experience of Hugin light years better."
  },
  {
    "title": "Updated website",
    "time": "2023 Q1",
    "description": "A new fancy website!"
  },
  {
    "title": "Multimedia sharing in Hugin",
    "time": "2023 Q2",
    "description": "Sharing images, videos, voice recordings and other media in chat apps is an absolute must these days. We've had such functionality in Hugin Messenger since 2019, but we are now looking to expand these functionalities into a scalable, easy to use and above all safe system that is portable between platforms."
  },
  {
    "title": "Hugin calls - Android mobile",
    "time": "2023 Q2",
    "description": "As of April 2023 users can enjoy voice & video calls in our Android app."
  },
  {
    "title": "Xeggex listing",
    "time": "2024 Q3",
    "description": "Kryptokrona was listed on Xeggex.com on the 27th of August 2023!"
  },
  {
    "title": "NonKYC.io listing",
    "time": "2024 Q4",
    "description": "Kryptokrona was listed on NonKYC.io on 16th July of 2024!"
  },
  {
    "title": "Aesir wallet",
    "time": "2024 Q4",
    "description": "Yggdrasil is a new state-of-the-art wallet that combines functionality and beauty. Stay tuned for this massive release!"
  },
  {
    "title": "CoinEx listing",
    "time": "2024 Q4",
    "description": "Kryptokrona was listed on CoinEx on 11/11!"
  },
  {
    "title": "P2P upgrade to Hugin Messenger",
    "time": "2024 Q4",
    "description": "Hugin Messenger is getting infinite scalability with a new architecture that sends all messages through P2P! This also enables file sharing and many other exciting new functionality!"
  },
  {
    "title": "Hugin + XKR Wallet on iOS",
    "time": "2025 Q1",
    "description": "With sideloading in iOS anyone can finally build Hugin Messenger for iOS and make sure the right code is running on the machine - which is an absolute necessety for users who demand uncompromized operational security."
  },
    {
    "title": "File sharing for Hugin Mobile",
    "time": "2025 Q1",
    "description": "With sideloading in iOS anyone can finally build Hugin Messenger for iOS and make sure the right code is running on the machine - which is an absolute necessety for users who demand uncompromized operational security."
  },
  {
    "title": "Tier 1 exchange",
    "time": "2025 Q2",
    "description": "We are already talking with some of the most popular exchanges with the hope of making a T1 listing possible ASAP!"
  }
]
